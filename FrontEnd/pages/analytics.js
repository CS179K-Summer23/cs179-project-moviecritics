...
function Analytics() {
  ...
  const [isMovieChartOpen, setIsMovieChartOpen] = useState(false); // State for Movie Chart
  const [pieData, setPieData] = useState([]);
  const [selectedAgeRange, setSelectedAgeRange] = useState(null);
  const [userData, setUserData] = useState([]);

  // New state variable for total movies
  ...
  const openMovieChartDialog = () => {
    setIsMovieChartOpen(true);
  };

  const closeMovieChartDialog = () => {
    setIsMovieChartOpen(false);
  };
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={4}>
            <div>
              ...
              {/* Replace 'dummyMovieData' with the actual movie data */}
              <BarChart width={300} height={200} data={dummyMovieData}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="label" />
                <YAxis />
                <Tooltip />
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Button variant="outlined" onClick={openMovieChartDialog}>
                  Open Bar Chart
                </Button>
                <Dialog open={isMovieChartOpen} onClose={closeMovieChartDialog}>
                  <DialogTitle>Bar Chart</DialogTitle>
                  <DialogContent>
                    {/* Replace 'dummyMovieData' with the actual movie data */}
                    <BarChart width={300} height={200} data={dummyMovieData}>
                  ...
                  <DialogActions>
                    <Button onClick={closeMovieChartDialog}>Close</Button>
                  </DialogActions>
                </Dialog>
              </div>
            </div>
