...
function Analytics() {
  ...
  const [isTotalMoviesChartOpen, setIsTotalMoviesChartOpen] = useState(false);
  ...
  const openTotalMoviesChartDialog = () => {
    setIsTotalMoviesChartOpen(true);
  };

  const closeTotalMoviesChartDialog = () => {
    setIsTotalMoviesChartOpen(false);
  };

  // Add this line to declare isTotalUsersChartOpen state
  const [isTotalUsersChartOpen, setIsTotalUsersChartOpen] = useState(false);

  const openTotalUsersChartDialog = () => {
    setIsTotalUsersChartOpen(true);
  };

  const closeTotalUsersChartDialog = () => {
    setIsTotalUsersChartOpen(false);
  };
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8003/totalMovies")
      .then((response) => {
        setTotalMovies(response.data.total_movies);
      })
      .catch((error) => {
        console.error("Error fetching total movies:", error);
      });
    
    // Fetch the total number of users - Add this part
    axios
      .get("http://localhost:8003/totalUsers")
      .then((response) => {
    ...
    <ThemeProvider theme={darkTheme}>
      ...
      >
        ...
        <div>
          ...
          <BarChart width={300} height={200} data={[{ label: "Total Movies", value: totalMovies }]}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="label" />
            <YAxis />
            <Tooltip />
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant="outlined" onClick={openTotalMoviesChartDialog}>
              Open Movies Chart
            </Button>
            <Dialog open={isTotalMoviesChartOpen} onClose={closeTotalMoviesChartDialog}>
              <DialogTitle>Movies Chart</DialogTitle>
              <DialogContent>
                <BarChart width={300} height={200} data={[{ label: "Total Movies", value: totalMovies }]}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="label" />
              ...
              <DialogActions>
                <Button onClick={closeTotalMoviesChartDialog}>Close</Button>
              </DialogActions>
            </Dialog>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            ...
            <Dialog open={isTotalUsersChartOpen} onClose={closeTotalUsersChartDialog}>
              ...
              <DialogActions>
                <Button onClick={closeTotalUsersChartDialog}>Close</Button>
              </DialogActions>
            </Dialog>
          </div>
        </div>
